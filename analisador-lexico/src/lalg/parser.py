...
class Parser:
    ...
    A gramática aceita programas com declarações de variáveis, procedimentos
    com parâmetros opcionais, comandos compostos, atribuições, chamadas,
    condicionais, repetição, leitura, escrita e expressões com precedência.
    O parser também mantém uma tabela de símbolos simples e recupera erros
    para continuar analisando o restante do arquivo.
    """
    ...
    def _variable_declarations(self) -> ASTNode:
        ...
        while not self._is_at_end() and not self._check_any({TokenType.PROCEDURE, TokenType.BEGIN}):
            if not self._check(TokenType.IDENTIFIER):
                ...
                continue
            declaration = self._single_variable_declaration()
